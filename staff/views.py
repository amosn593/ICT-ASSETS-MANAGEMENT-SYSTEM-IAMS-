    ...
    if request.method == "GET":
        ...
        try:
            data = Comp.objects.filter(
                client__staff_number__icontains=client_number, client__asset_assigned='Yes')[:1]

            dat = Comp.objects.filter(client__staff_number__icontains=client_number,
                                      client__asset_assigned='Yes', ict_approval='Approved')
            num = dat.count()
        except:
            data = []
    ...
    if request.method == "GET":
        ...
        try:
            data = Repair.objects.filter(
                comp__client__staff_number=usernumber).order_by('-cdate', '-rdate')
        except:
            data = []
        ...
        if Comp.objects.filter(asset_serial__icontains=asset_serial).count() > 0:
            ...
            if Client.objects.filter(comp__asset_serial__icontains=asset_serial, staff_number__icontains=staff_number).count() > 0:
                ...
                c = Client.objects.get(
                    comp__asset_serial__icontains=asset_serial, staff_number__icontains=staff_number)
                ...
                try:
                    ...
                    comp_id = Comp.objects.get(
                        asset_serial__icontains=asset_serial)
                    r = Requested(request_number=number, request_service=service,
                                  request_reason=reason, request_date=date, comp=comp_id)
                    r.save()

                    # Flash message and redirect to home page
                    messages.success(
                        request, f"Service Request send successfully!!")
                    return redirect('staff_home')

                except:
                    # Flash message and redirect to home page
                    messages.success(
                        request, f"Check your internet connection and try again!!")
                    return redirect('staff_home')
            ...
            # Flash message and redirect to home page
            messages.success(
                request, f"No such Asset serial Number in the database!!")
            return redirect('staff_home')
    else:
        return HttpResponse("INVALID HTTP METHOD")
...
def my_asset_approvals(request):
    if request.method == 'GET':
        ...
        try:
            dat = Comp.objects.exclude(ict_approval='Approved').filter(
                client__staff_number__icontains=client_number, client__asset_assigned='Yes')
        except:
            dat = ""
...
def hod_approvals(request):
    if request.method == 'GET':
        ...
        try:
            dat = Comp.objects.filter(client__hod__hod_number__icontains=client_number,
                                      client__asset_assigned='Yes', client__hod_approval='Pending')
        except:
            dat = []
    ...
    if request.method == 'GET':
        ...
        if Client.objects.get(comp__asset_serial=serial).hod_approval == 'Approved':
            ...
            if Client.objects.get(comp__asset_serial=serial).accepted == 'Pending':
                Client.objects.filter(comp__asset_serial=serial).update(
                    accepted='Accepted')

                # email_ict(serial)
                # Flash message and redirect to home page
                messages.success(
                    request, f"Successfully accepted Ict Asset Assigned to you!!!")
                return redirect('my_asset_approvals')
            else:
                # email_ict(serial)
                # Flash message and redirect to home page
                messages.info(
                    request, f"You have already accepted or rejected Ict Asset Assigned to you!!!")
                return redirect('my_asset_approvals')
        else:
            # Flash message and redirect to home page
            messages.info(
                request, f"Your HOD has not Approved Ict Asset Assigned to you!!!")
            return redirect('my_asset_approvals')
    else:
        return HttpResponse("INVALID HTTP METHOD")
# HOD asset approval
...
def hod_asset_approval(request):
    if request.method == 'GET':
        ...
        if Client.objects.get(comp__asset_serial=serial).hod_approval == 'Pending':
            ...
            # Flash message and redirect to home page
            messages.success(
                request, f"Successfully approved Ict Asset Assigned to your staff!!!")
            return redirect('hod_approvals')
        else:
            # Flash message and redirect to home page
            messages.info(
                request, f"You have already Approved Ict Asset Assigned to your staff!!!")
            return redirect('hod_approvals')
    else:
        return HttpResponse("INVALID HTTP METHOD")
